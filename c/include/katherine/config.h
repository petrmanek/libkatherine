...
KATHERINE_EXPORTED int katherine_trigger_snprint(char *buf, size_t cap,
                                                 const katherine_trigger_t *v);

typedef struct katherine_test_pulse_config {
  bool enabled; ///< true if test pulses should be injected during acquisition
  bool digital_only; ///< false: pulse the analog frontend (amplitude =
                     ///< VTP_coarse - VTP_fine, DAC LSBs 5 mV / 2.5 mV), true:
                     ///< pulse the digital discriminator input
  bool external; ///< set true to bypass internal pulse generator with signal
                 ///< from the ExtTPulse pad (in such case count/period/phase do
                 ///< not apply)
  uint16_t count;  ///< number of pulses per shutter opening, 1 to 65535
  uint16_t period; ///< pulse period in pixel-clock cycles (25 ns at 40 MHz), 65
                   ///< to 16321 (1.625 us to 408.025 us), rounded down to the
                   ///< nearest 64k + 1
  uint8_t phase; ///< pulse edge clock phase, selects Clk_ph_shift[0..15]; keep
                 ///< 0 unless a multi-phase pixel clock is configured
} katherine_test_pulse_config_t;

KATHERINE_EXPORTED int
katherine_test_pulse_config_snprint(char *buf, size_t cap,
                                    const katherine_test_pulse_config_t *v);
...
KATHERINE_EXPORTED const char *katherine_str_phase(katherine_phase_t phase);
...
KATHERINE_EXPORTED const char *katherine_str_freq(katherine_freq_t freq);
...
KATHERINE_EXPORTED int katherine_config_snprint(char *buf, size_t cap,
                                                const katherine_config_t *v);

KATHERINE_EXPORTED int katherine_configure(katherine_device_t *device,
                                           const katherine_config_t *config);

KATHERINE_EXPORTED int
katherine_set_all_pixel_config(katherine_device_t *device,
                               const katherine_px_config_t *px_config);

KATHERINE_EXPORTED int katherine_set_acq_time(katherine_device_t *device,
                                              double ns);

KATHERINE_EXPORTED int
katherine_set_acq_mode(katherine_device_t *device,
                       katherine_acquisition_mode_t acq_mode,
                       bool fast_vco_enabled);

KATHERINE_EXPORTED int katherine_set_no_frames(katherine_device_t *device,
                                               int no_frames);

KATHERINE_EXPORTED int katherine_set_bias(katherine_device_t *device,
                                          unsigned char bias_id,
                                          float bias_value);
...
KATHERINE_EXPORTED int katherine_acquisition_setup(
    katherine_device_t *device, const katherine_trigger_t *start_trigger,
    bool delayed_start, const katherine_trigger_t *end_trigger);
...
KATHERINE_EXPORTED int katherine_set_sensor_register(katherine_device_t *device,
                                                     char reg_idx,
                                                     int32_t reg_value);
...
KATHERINE_EXPORTED int katherine_timer_set(katherine_device_t *device);

KATHERINE_EXPORTED int katherine_set_dacs(katherine_device_t *device,
                                          const katherine_dacs_t *dacs);

KATHERINE_EXPORTED int
katherine_set_test_pulses(katherine_device_t *device,
                          const katherine_test_pulse_config_t *tp_config);
